...
RFD_transmitter::RFD_transmitter(const char *rmt_ip, const char *rmt_port, RFD_tmr *tmr)
      : RFD_interface("RFDtx", 0),
        ...
        tmr(tmr)
{
  // Create RFD socket and bind to local tx_port and remote hostname:rx_port
  fd = socket(AF_INET, SOCK_DGRAM, 0);
  if (fd < 0) msg(3, "Unable to create RFD socket: %s", strerror(errno));
  ...
  */
  ...
  pkt = (RFDdiag_packet*)new_memory(max_packet_size);
...
//   XQ     Remote Quit
bool RFD_transmitter::parse_command(char *cmd, unsigned cmdlen) {
  if (cmd && cmdlen <= 7) {
...
bool RFD_transmitter::transmit(uint16_t n_pkts) {
  bool rv;
  for (int i = 0; i < n_pkts; ++i) {
    ...
    pkt->Packet_size = sizeof(RFDdiag_packet) + L2R_command_len;
    ...
    pkt->Receive_SN = RFDdiag.R2L.Receive_SN;
    pkt->Int_packets_rx = RFDdiag.R2L.Int_packets_rx;
    pkt->Int_min_latency = RFDdiag.R2L.Int_min_latency;
    pkt->Int_mean_latency = RFDdiag.R2L.Int_mean_latency;
    pkt->Int_max_latency = RFDdiag.R2L.Int_max_latency;
    pkt->Int_bytes_rx = RFDdiag.R2L.Int_bytes_rx;
    pkt->Int_bytes_tx = RFDdiag.L2R.Int_bytes_tx;
    pkt->Total_valid_packets_rx = RFDdiag.R2L.Total_valid_packets_rx;
    pkt->Total_invalid_packets_rx = RFDdiag.R2L.Total_invalid_packets_rx;
...
bool RFD_transmitter::tm_sync_too() {
...
RFD_receiver::RFD_receiver(const char *port, bool allow_remote_commands,
                            RFD_transmitter *tx)
      : RFD_interface("RFDrx", 10000),
        recv_port(port),
        ...
        R2L_latencies(0)
{
  // Create RFD socket and bind to local port
  fd = socket(AF_INET, SOCK_DGRAM, 0);
  if (fd < 0)
    msg(3, "%s: Unable to create RFD socket: %s",
...
bool RFD_receiver::protocol_input() {
  bool rv = false;
  int32_t now = get_timestamp();
  
  ++R2L_Total_packets_rx;
  if (nc < sizeof(RFDdiag_packet)) {
    ++R2L_Total_invalid_packets_rx;
    size_t expected = sizeof(RFDdiag_packet);
    if (nc < offsetof(RFDdiag_packet, Transmit_SN))
  ...
  if (pkt->Packet_size != nc ||
      sizeof(RFDdiag_packet) + pkt->Command_bytes > pkt->Packet_size) {
    ++R2L_Total_invalid_packets_rx;
    report_err("%s: Packet_size(%u) != nc(%u) or minsize(%u)+Cmd(%d) > Packet_size",
      iname, pkt->Packet_size, nc, sizeof(RFDdiag_packet), pkt->Command_bytes);
    consume(nc);
    return false;
  }
  if (!crc_ok()) {
...
void RFD_tmr::set_transmitter(RFD_transmitter *tx) {
  this->tx = tx;
}
...
int main(int argc, char **argv) {
  ...
  ELoop.add_child(tmr);
  RFD_transmitter *tx = new RFD_transmitter(remote_ip, tx_port, tmr);
  ELoop.add_child(tx);
  RFD_receiver *rx = new RFD_receiver(rx_port, allow_remote_commands, tx);
  ELoop.add_child(rx);
